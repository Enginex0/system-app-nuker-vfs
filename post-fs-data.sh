...
# --- mounting mode handling ---
if [ "$mounting_mode" = "3" ]; then # on nomount vfs
    # VFS mode - no overlay mounting needed
    # NoMount module handles VFS redirections via metamount.sh
    # We just ensure skip_mount is set so manager doesn't auto-mount
    echo "app_nuker_debug: post-fs-data: nomount vfs mode (undetectable)..." >> /dev/kmsg
    touch "$MODDIR/skip_mount"
    touch "$MODDIR/skip_mountify"
    # whiteouts in system/ dir will be processed by NoMount's metamount.sh
elif [ "$mounting_mode" = "2" ]; then # on mountify
